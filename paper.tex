...
\subsection{Deep-MOND regime}

The exponential interpolation has the deep-MOND limit
$g_{\rm obs} \to g_{\rm bar}^{1-p} \cdot a_0^p$. Standard MOND requires
$p = 0.5$ to recover $g_{\rm obs} = \sqrt{g_{\rm bar} a_0}$. As
$p \to 0$ (very low $\Sigma$), the formula degenerates to
$g_{\rm obs} \to g_{\rm bar}$ (Newtonian).

We implemented \citet{McGaugh2016Crater}'s exact EFE formalism:
$\sigma_{\rm iso}^4 = (4/81)\,G\,M\,a_0$ and
$\sigma_{\rm efe}^2 = a_0\,G\,M / (3\,g_{\rm ex}\,r_{1/2})$. This
reproduces the published Crater II prediction $\sigma = 2.06\,
{\rm km}\,{\rm s}^{-1}$. For $p(\Sigma)$, the prediction is much
lower ($\sim 0.6\,{\rm km}\,{\rm s}^{-1}$), inconsistent with the
observed $2.7 \pm 0.3\,{\rm km}\,{\rm s}^{-1}$. We conclude that
$p(\Sigma)$ is calibrated to SPARC's transition regime ($x \sim 1$)
and does not extrapolate to deep MOND ($x \ll 1$). The standard
McGaugh+EFE formulation remains correct for ultra-faint dwarfs.

\subsection{Other limitations}

$\alpha$ has a plateau ($\alpha \in [5/3, 1.88]$). $\beta = 0.4$ is
empirical. EFE corrections must be applied separately. The CDT
structural connection is suggestive, not mechanistic.
