...
- **Scherer, Pflamm-Altenburg, Kroupa & Gjergo (2025, A&A 698, A167, [arXiv:2504.17002](https://arxiv.org/abs/2504.17002))**: p-Laplacian framework with density-dependent exponent. They use **volume density ρ** (not Σ) in a generalized Poisson equation `∇·(|∇Φ|/a_0)^(p-2)∇Φ = 4πGρ`, modifying the kinetic operator. Their p ranges 2-12+ (Newtonian to deep MOND to cosmological). Their fit is `ρ(p) = α exp(βp)` (exponential, **not sigmoid**), and their characteristic scale is the cosmological ρ_crit ≈ 1.4×10⁻⁷ M☉/Mpc³, **unrelated to a_0**. Their p extrapolated to ρ_crit is 12.27 ± 0.39 — diverging, not bounded in [0, 2/3]. Conceptually adjacent (parameter varies with density) but **structurally distinct** from p(Σ).
- **Milgrom (2016, [arXiv:1607.05103](https://arxiv.org/abs/1607.05103))**: Central Surface Density Relation Σ_D⁰ = Σ_M·S(Σ_B⁰/Σ_M), with **Σ_M = a_0/(2πG) ≈ 138 M☉/pc²**. The factor 2π is the thin-disk one-sided Gauss law (g_N⁺ = 2πGΣ at g_N⁺ = a_0). Σ enters as the *argument* of a fixed ν-function S(y) = ∫₀^y ν(y')dy' — universal shape, not Σ-dependent. **Σ_0 = a_0/(4π²G) = Σ_M/(2π)** (this work) is the mean-Σ analog of Milgrom's central CSDR scale; the 2π factor is geometric (mean-vs-central averaging in exponential disks with R_last/R_d ≈ 3.4). p(Σ) extends Milgrom's CSDR by allowing the function shape itself to depend on Σ.
