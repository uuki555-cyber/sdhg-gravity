...
- The exponent 1/3 in the formula is approximate; the global fit gives alpha = 0.31 (6% below 1/3). A self-built 2+1D CDT simulation (with dynamic volume) gives gamma = 0.340 ± 0.025 (consistent with 1/3 at 0.3 sigma). However, published 4D CDT data (approximate, read from figures) suggests gamma ~ 1.1 when fitting with free dUV, and the 4D case remains unverified due to lack of precise numerical data
- The functional form p(M) = 2u/(1+3u) is mathematically identical to the CDT spectral dimension formula d = dUV + (dIR-dUV)*u/(1+u). The 2+1D CDT connection is numerically supported; the 4D connection is unresolved
