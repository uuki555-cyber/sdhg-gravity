...
- The exponent 1/3 in the formula is approximate; the global fit gives alpha = 0.31 (6% below 1/3). A self-built 2+1D CDT simulation (`run_cdt_2plus1d.py`, L=10, T=15, 200 sweeps, pure Python ~30 min) gives gamma = 0.340 ± 0.025 (seed variance only). Systematic uncertainties not included: (a) temporal link structure is ad-hoc (not proper CDT sandwiches), (b) single lattice size (no finite-size scaling to L→∞), (c) d_UV=1.5 fixed in fit. A numba-accelerated version (`cdt_fast.py`) achieves ~100x MC speedup but has an unresolved volume instability at the CDT critical point
