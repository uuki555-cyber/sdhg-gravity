...
- The exponent 1/3 in the formula is approximate; the global fit gives alpha = 0.31 (6% below 1/3). **CDT simulation status**: `run_cdt_2plus1d.py` evolves spatial slices independently and connects them with ad-hoc temporal links; this is NOT proper CDT (which requires spacetime Pachner moves on a single simplicial complex). The earlier gamma = 0.340 ± 0.025 result has an unquantified systematic from this temporal link structure. A C-accelerated version (`cdt_sim.c`, 1000x faster MC) confirmed the temporal link problem: the spectral dimension does not show clean dimensional flow with the current graph construction. Proper 2+1D CDT requires (2,3)/(3,2) spacetime Pachner moves, which is a separate implementation project
...
- The functional form p(M) = 2u/(1+3u) is mathematically identical to the CDT spectral dimension formula d = dUV + (dIR-dUV)*u^γ/(1+u^γ). The 2+1D connection is unverified pending proper CDT implementation; the 4D connection is open
