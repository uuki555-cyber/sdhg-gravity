...
- The exponent 1/3 in the formula is approximate; the global fit gives alpha = 0.31 (6% below 1/3). **CDT simulation** (`cdt_pachner.py`): A proper 2+1D CDT implementation with spacetime (2,3)/(3,2) Pachner moves shows spectral dimension flowing from ~1.8 (short distance) to ~2.8 (continuum), consistent with 3D spacetime. However, the lattice is too small (~10⁴ tetrahedra) to distinguish true UV quantum gravity effects from lattice artifacts; the "flow" observed is from lattice-scale to continuum-scale, not from UV dimension reduction. Measuring gamma requires N_tets >> 10⁶ with Regge action in the physical CDT phase (phase C), which is beyond the current implementation
