...
- The exponent 1/3 in the formula is approximate; the global fit gives alpha = 0.31 (6% below 1/3). **CDT simulation** (`cdt_pachner.py`): A proper 2+1D CDT implementation with spacetime (2,3)/(3,2) Pachner moves confirms dimensional flow from d_UV ≈ 2.0 (UV) to d_IR ≈ 2.8-3.0 (IR), as expected for 2+1D quantum gravity. The gamma exponent cannot yet be reliably extracted due to d_spec noise at available lattice sizes (L=10, T=15). Larger lattices with volume control (cosmological constant term) are needed for a clean gamma measurement
